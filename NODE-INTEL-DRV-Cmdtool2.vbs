...
Const ForAppending = 8
  ...
  crt.Screen.Send "-------------------------------------------------------" & vbcr & vblf , True
  crt.Screen.Send " /opt/MegaRAID/CmdTool2/CmdTool2 -PDList -a0 | egrep  'Enclosure Device ID|Slot|Media Error|Other Error|Predictive Failure Count|Firmware state\(0\)' | paste - - - - -" & vbcr
  crt.Screen.Send ""
...
'#################################
  ...
  crt.Screen.Send "-------------------------------------------------------" & vbcr & vblf , True
  crt.Screen.Send " /opt/MegaRAID/CmdTool2/CmdTool2 -LDPDinfo -a0 | grep '^\(Virtual Drive\|State\|Number Of Drives\)' | paste - - -" & vbcr
  crt.Screen.WaitForString chr(10) 
  Do
    result = crt.Screen.WaitForStrings( waitStrs )
    If result = 2 Then
...
'###################################
  ...
  crt.Screen.Send "-------------------------------------------------------" & vbcr & vblf , True
  crt.Screen.Send " /opt/MegaRAID/CmdTool2/CmdTool2 -LdPdInfo -aAll | egrep '^Adapter|^Number of Virtual|^Virtual Drive:|^Name|^Enclosure Device ID:|^Slot Number:|^Inquiry Data:' | grep -v 'Adapter'" & vbcr
  crt.Screen.WaitForString chr(10)
  Do	
    result = crt.Screen.WaitForStrings( waitStrs )
    If result = 2 Then
